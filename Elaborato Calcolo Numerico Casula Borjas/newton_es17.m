...
if(length(X)~=length(Y)), error('Il numero di ascisse di interpolazione e di valori della funzione non è uguale!'),
end
if (length(X) ~= length(unique(X))), error('Le ascisse di interpolazione non sono distinte!'),
